...
# %% auto #0
__all__ = ['PreEncodedHMEPDataset', 'ChunkShuffleSampler', 'collate_emb_levels', 'get_pos_cache', 'emb_levels_to_enc_out',
           'mask_fine_levels', 'hmep_loss', 'train_hmep', 'train_hmep_main']
...
# %% ../nbs/13_train_hmep.ipynb #3jt8tozsu6y
import glob
from torch.utils.data import Dataset, DataLoader, Sampler

class PreEncodedHMEPDataset(Dataset):
    """Load pre-encoded Swin embeddings (emb1 anchor) for HMEP training.
    Expects files named {split}_chunk*.pt in encoded_dir, each a list of dicts with 'emb1' key.
    Each dict holds a batched record: emb1[level] has shape (B_enc, N, D).
    This dataset flattens to individual samples. Loads chunks lazily."""
    def __init__(self, encoded_dir, split='train'):
        self.files = sorted(glob.glob(os.path.join(encoded_dir, f'{split}_chunk*.pt')))
        assert self.files, f"No {split}_chunk*.pt files found in {encoded_dir}"
        self.index = []              # (chunk_idx, record_idx, sample_idx_within_record)
        self.chunk_sample_ranges = {}  # ci -> (start, end) in self.index
    ...
    def __getitem__(self, idx):
        ...
        if ci != self._cache_ci:
            ...
            self._cache_ci = ci
        return [lvl[si].float() for lvl in self._cache_data[ri]['emb1']]  # list of 6 tensors, each (N, D)


class ChunkShuffleSampler(Sampler):
    """Shuffle chunk order each epoch, but access items within each chunk sequentially.
    Prevents workers from thrashing between chunks while still randomizing order."""
    def __init__(self, dataset: PreEncodedHMEPDataset, shuffle=True):
    ...
    def __iter__(self):
        n_chunks = len(self.dataset.files)
        chunk_order = torch.randperm(n_chunks).tolist() if self.shuffle else list(range(n_chunks))
        indices = []
        for ci in chunk_order:
            start, end = self.dataset.chunk_sample_ranges[ci]
            indices.extend(range(start, end))
        return iter(indices)


def collate_emb_levels(batch):
    "Stack per-sample level embeddings, padding variable-N levels to max size."
    n_levels = len(batch[0])
    result = []
    for lev in range(n_levels):
        tensors = [item[lev] for item in batch]  # each (N, D)
        ...
        for t in tensors:
            ...
            padded.append(t)
        result.append(torch.stack(padded))  # (B, N, D)
    return result


def get_pos_cache(encoder, image_size, device):
    "Run encoder on a dummy image to extract per-level pos tensors. Returns (N, 2) per level."
    dummy = torch.zeros(1, 1, image_size, image_size, device=device)
    with torch.no_grad():
        enc_out = encoder(dummy)
    # squeeze batch dim: HMEP forward expects pos as (N, 2), handles batching itself
    return [lvl.pos.squeeze(0) for lvl in enc_out.patches.levels]


def emb_levels_to_enc_out(emb_levels, pos_cache, device):
    "Assemble a minimal EncoderOutput from stored emb tensors and cached pos."
    levels = [
        PatchState(emb=emb.to(device), pos=pos_cache[i],
                   non_empty=None, mae_mask=None)
...
def train_hmep(cfg: DictConfig):
    ...
    if use_preencoded:
        ...
        train_ds = PreEncodedHMEPDataset(encoded_dir, split='train')
        val_ds   = PreEncodedHMEPDataset(encoded_dir, split='val')
