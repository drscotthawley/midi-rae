...
# %% auto #0
__all__ = ['SCHEME_NAMES', 'TARGET_NAMES', 'shift_no_wrap', 'sample_shift', 'note_length_weights', 'resolve_split_files',
           'AnchorDataset', 'sample_shifts', 'PRPairDataset', 'ShiftedTripletDataset', 'PreEncodedChunkDataset',
           'ChunkShuffleSampler', 'collate_emb_levels', 'collate_preencode', 'get_pos_cache', 'emb_levels_to_enc_out',
           'EmbeddingDataset', 'ConditionalFlowDataset', 'ConditionalFlowChunkSampler', 'DINOv2Dataset']
...
# %% ../nbs/01_data.ipynb #b7f1c2d4-split-resolver-cell
def resolve_split_files(image_dataset_dir, split='train', val_fraction=0.1, seed=42, verbose=True):
    """Return the list of image files belonging to `split`.

    Prefers an EXPLICIT on-disk split: if <dir>/train and <dir>/val both exist, the
    files in <dir>/<split>/ are used as-is. This is the reliable path -- the split is
    a property of the data, identical on every machine.

    Falls back to a seeded shuffle when those subdirectories are absent, but sorts the
    glob first. Unsorted, glob() returns filesystem order, which differs per machine:
    the same 909 files gave three different train/val partitions across three hosts,
    silently making their val losses and 'best' checkpoints incomparable.
    """
    root = os.path.expandvars(os.path.expanduser(str(image_dataset_dir)))
    if all(os.path.isdir(os.path.join(root, s)) for s in ('train', 'val')):
        files = sorted(glob(os.path.join(root, split, '**/*.png'), recursive=True))
        if verbose: print(f"explicit {split}/ split: {len(files)} files")
        return files
    files = sorted(glob(os.path.join(root, '**/*.png'), recursive=True))
    rng = random.Random(seed)
    shuffled = files.copy()
    rng.shuffle(shuffled)
    cut = int(len(shuffled) * (1 - val_fraction))
    if verbose: print(f"no train//val dirs under {root}; using seeded shuffle for {split}")
    return shuffled[:cut] if split == 'train' else shuffled[cut:]
...
class AnchorDataset(Dataset):
    ...
    def __init__(self,
        ...
        # Load filenames for this split
        self.filenames = resolve_split_files(image_dataset_dir, split, val_fraction, seed)
