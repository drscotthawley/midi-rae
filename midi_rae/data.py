...
# %% auto #0
__all__ = ['SCHEME_NAMES', 'TARGET_NAMES', 'shift_no_wrap', 'sample_shift', 'AnchorDataset', 'sample_shifts', 'PRPairDataset',
...
class AnchorDataset(Dataset):
    ...
    def __init__(self, 
        ...
        for f in self.filenames: 
            ...
            self.images.append((np.array(img, dtype=np.uint8) > 0).astype(np.uint8))
...
class PRPairDataset(Dataset):
    ...
    def __init__(self, 
        ...
        for f in self.filenames: 
            img = Image.open(f).convert('L')  # grayscale
            self.images.append(np.array(img, dtype=np.uint8)) # store as binary
        if verbose: print(f"Finished loading.")
