...
def apply_mae_mask(x, pos, pmask, mask_ratio=0.75, has_cls_token=True):
    "Apply token masking for MAE training. 1=keep, 0=masked"
    mae_mask = pmask[0] & (torch.rand(pmask.shape[1], device=pmask.device) > mask_ratio)  #  (B, N), 1=visible, 0=masked
    if has_cls_token: mae_mask[0] = True  
    return x[:, mae_mask, :], pos[mae_mask, :], pmask[:,mae_mask], mae_mask 
...
class ViTEncoder(nn.Module):
    ...
    def forward(self, x, return_cls_only=True, mask_ratio=0.75):
        ...
        x, pos_visible, pmask_visible, mae_mask = apply_mae_mask(x, pos, pmask, mask_ratio=mask_ratio if self.training else 0.0)
