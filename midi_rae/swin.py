...
# %% auto #0
__all__ = ['SwinEncoder', 'PixelShuffleHead', 'SwinMAEDecoder', 'PatchExpand', 'SwinDecoder', 'SwinMaskedEmbeddingPredictor',
           'ConvBlock', 'CNNSwinEncoder']
...
# %% ../nbs/10_swin.ipynb #a6feb5f2-726d-41bf-b6eb-193f9ce66065
class SwinMaskedEmbeddingPredictor(nn.Module):
    ...
    externally on masked positions only, e.g.:
        preds, masks = predictor(enc_out_context)
        targets = enc_out_target.patches.levels
        loss = sum(((p[~m] - t.emb[~m])**2).mean()
                   for p, m, t in zip(preds, masks, targets))
    ...
    def __init__(self, dims=(256, 128, 64, 32, 16, 8), summary_dim=128,
        ...
        if delta_hidden_dim is None: delta_hidden_dim = summary_dim
        self.delta_proj = nn.Linear(2, 2 * delta_hidden_dim)
    ...
    def _level_mask_ratios(self):
        """Per-level mask ratios (coarsest-first). Finest gets self.mask_ratio,
        each coarser level divides by mr_level_fac."""
        return [self.mask_ratio / (self.mr_level_fac ** (self.n_levels - 1 - i))
                for i in range(self.n_levels)]

    def _make_level_masks(self, levels, device):
        """Generate per-level random masks.
        Returns: list of (B, N_i) bool tensors, True=visible, False=masked."""
    ...
    def forward(self, enc_out: EncoderOutput, mask_ratio: Optional[float] = None,
                deltas: Optional[Union[int, float, torch.Tensor]] = None,
                ) -> tuple[list[torch.Tensor], list[torch.Tensor]]:
        """
        Args:
            enc_out: EncoderOutput from SwinEncoder
            mask_ratio: override finest-level mask ratio (0 = no masking)
        Returns:
            (preds, masks): preds is list of (B, N_i, D_i) per level (coarsest-first);
            masks is list of (B, N_i) bool (True=visible). Compute loss on ~mask positions.
        """
        levels = enc_out.patches.levels
        B, device = levels[0].emb.shape[0], levels[0].emb.device
        if deltas is not None and not isinstance(deltas, torch.Tensor):
            deltas = torch.zeros(B, 2, device=device)  # (B, 2) for (dx, dy)
        ...
        if deltas is not None:
            delta_emb = self.delta_proj(deltas.float() / self.max_shift)
            gamma, beta = delta_emb.chunk(2, dim=-1)
            summaries = summaries * (1 + gamma.unsqueeze(1)) + beta.unsqueeze(1)
...
class ConvBlock(nn.Module):
    """Conv2d block as drop-in replacement for a SwinTransformerV2Stage (no downsampling).
    Expects NHWC input, returns NHWC output, same as Swin stages."""
    def __init__(self, dim, depth=2):
        super().__init__()
        layers = []
        for i in range(depth):
            layers += [
                nn.GroupNorm(1, dim),  # equivalent to LayerNorm over channels
                nn.Conv2d(dim, dim, kernel_size=3, padding=1),
                nn.GELU(),
            ]
        self.block = nn.Sequential(*layers)
        # Stub so the stage loop's `stage.downsample(x)` / `stage(x)` both work
        self.downsample = nn.Identity()

    def forward(self, x):
        # x: (B, H, W, C) NHWC — convert to NCHW for Conv2d, then back
        x = x.permute(0, 3, 1, 2).contiguous()       # → NCHW
        x = self.block(x)
        return x.permute(0, 2, 3, 1).contiguous()     # → NHWC


class CNNSwinEncoder(SwinEncoder):
    """SwinEncoder with finest stage replaced by a ConvBlock (no attention)."""
    def __init__(self, *args, conv_depth=2, **kwargs):
        super().__init__(*args, **kwargs)
        # Replace stage 0 (finest level) with ConvBlock
        self.stages[0] = ConvBlock(dim=self.embed_dim, depth=conv_depth)
