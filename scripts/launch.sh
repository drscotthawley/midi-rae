...
#   tag    — short descriptive label (e.g. "lr2e4"); a 6-char random suffix is appended
#
# Example:
#   ./scripts/launch.sh lecun enc config_swin lr2e4

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_DIR="$(dirname "$SCRIPT_DIR")"

HOST="${1:?Usage: $0 <host> <enc|dec> <config> <tag>}"
TYPE="${2:?Usage: $0 <host> <enc|dec> <config> <tag>}"
CONFIG="${3:?Usage: $0 <host> <enc|dec> <config> <tag>}"
TAG="${4:?Usage: $0 <host> <enc|dec> <config> <tag>}"
...
$SSH "${HOST}" "mkdir -p ${RUN_DIR}/midi_rae ${RUN_DIR}/configs"

# Copy source snapshot and config files
echo "Copying midi_rae/*.py and configs/ to ${HOST}:${RUN_DIR}/ ..."
scp "${REPO_DIR}"/midi_rae/*.py "${HOST}:${RUN_DIR}/midi_rae/"
scp "${REPO_DIR}/configs/${CONFIG}.yaml" "${HOST}:${RUN_DIR}/configs/"
